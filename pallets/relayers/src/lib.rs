    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn register(origin: OriginFor<T>, valid_till: BlockNumberFor<T>) -> DispatchResult {
            ...
            RegisteredRelayers::<T>::try_mutate(&relayer, |maybe_registration| -> DispatchResult {
                let mut registration = maybe_registration.unwrap_or_else(|| Registration {
                    valid_till,
                    stake: Zero::zero(),
                });
        ...
        pub fn deregister(origin: OriginFor<T>) -> DispatchResult {
            ...
            RegisteredRelayers::<T>::try_mutate(&relayer, |maybe_registration| -> DispatchResult {
                ...
                Self::deposit_event(Event::<T>::Deregistered {
                    relayer: relayer.clone(),
                });
...
mod tests {
    ...
    fn deregister_works() {
        run_test(|| {
            ...
            let free_balance = Balances::free_balance(REGISTER_RELAYER);
            assert_ok!(Pallet::<TestRuntime>::deregister(RuntimeOrigin::signed(
                REGISTER_RELAYER
            )));
            assert_eq!(
                Balances::reserved_balance(REGISTER_RELAYER),
            ...
            assert_eq!(
                Balances::free_balance(REGISTER_RELAYER),
                free_balance + Stake::get()
            );
            ...
            assert_eq!(
                ...
                Some(&EventRecord {
                    phase: Phase::Initialization,
                    event: TestEvent::Relayers(Event::Deregistered {
                        relayer: REGISTER_RELAYER
                    }),
                    topics: vec![],
                }),
    ...
    fn is_registration_active_is_false_for_unregistered_relayer() {
        run_test(|| {
            assert!(!Pallet::<TestRuntime>::is_registration_active(
                &REGISTER_RELAYER
            ));
        });
    }
