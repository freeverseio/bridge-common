...
macro_rules! generate_bridge_reject_obsolete_headers_and_messages {
	($call:ty, $account_id:ty, $($filter_call:ty),*) => {
		#[derive(Clone, parity_scale_codec::Decode, Default, parity_scale_codec::Encode, Eq, PartialEq, frame_support::RuntimeDebug, scale_info::TypeInfo)]
...
/// Since `NetworkId` doesn't include `Evochain`, `Rococo` and `OwnershipChain`, we create some
...
pub enum CustomNetworkId {
    ...
    Rococo,
    /// The Ownership network ID, associated with Westend.
    OwnershipChain,
}
...
impl TryFrom<bp_runtime::ChainId> for CustomNetworkId {
    ...
    fn try_from(chain: bp_runtime::ChainId) -> Result<Self, Self::Error> {
        ...
        if chain == *b"evol" {
            ...
            Ok(Self::Rococo)
        } else if chain == *b"ownr" {
            Ok(Self::OwnershipChain)
        } else {
            Err(())
        }
    }
...
impl CustomNetworkId {
    ...
    pub const fn as_network_id(&self) -> NetworkId {
        match *self {
            ...
            CustomNetworkId::OwnershipChain => NetworkId::Westend,
        }
    }
}
