        ...
        + MessagesCallSubType<Runtime, Msgs::Instance>,
{
    ...
    fn parse_and_check_for_obsolete_call(
        ...
        let msgs_call = calls
            .next()
            .transpose()?
            .and_then(|c| c.call_info_for(Msgs::Id::get()));
        let para_finality_call = calls
            .next()
            .transpose()?
            .and_then(|c| c.submit_parachain_heads_info_for(Para::Id::get()));
        let relay_finality_call = calls
            .next()
            .transpose()?
            .and_then(|c| c.submit_finality_proof_info());
        ...
        Ok(
            match (
                ...
                (3, Some(relay_finality_call), Some(para_finality_call), Some(msgs_call)) => {
                    Some(CallInfo::AllFinalityAndMsgs(
                        relay_finality_call,
                        para_finality_call,
                        msgs_call,
                    ))
                }
                (2, None, Some(para_finality_call), Some(msgs_call)) => Some(
                    CallInfo::ParachainFinalityAndMsgs(para_finality_call, msgs_call),
                ),
    ...
    fn bundled_messages_for_priority_boost(call_info: Option<&CallInfo>) -> Option<MessageNonce> {
        ...
        let bundled_messages = parsed_call
            .messages_call_info()
            .bundled_messages()
            .checked_len()
            .unwrap_or(0);
...
mod tests {
    ...
    fn initialize_environment(
        ...
        let in_lane_data = InboundLaneData {
            last_confirmed_nonce: best_message,
            ..Default::default()
        };
        pallet_bridge_messages::InboundLanes::<TestRuntime>::insert(lane_id, in_lane_data);

        let out_lane_data = OutboundLaneData {
            latest_received_nonce: best_message,
            ..Default::default()
        };
    ...
    fn validate_boosts_priority_of_message_delivery_transactons() {
        run_test(|| {
            ...
            let priority_of_200_messages_confirmation =
                run_validate(message_confirmation_call(300))
                    .unwrap()
                    .priority;
            assert_eq!(
    ...
    fn post_dispatch_ignores_transaction_that_has_not_delivered_any_messages() {
        run_test(|| {
            ...
            assert_storage_noop!(run_post_dispatch(
                Some(delivery_pre_dispatch_data()),
                Ok(())
            ));
            ...
            assert_storage_noop!(run_post_dispatch(
                Some(confirmation_pre_dispatch_data()),
                Ok(())
            ));
        });
    }
